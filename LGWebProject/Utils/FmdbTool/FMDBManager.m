...
-(void)DataBase:(FMDatabase *)db createTable:(NSString *)tableName keyTypes:(NSDictionary *)keyTypes
{
    if ([self isOpenDatabese:db]) {
        ...
        NSMutableString *sql = [[NSMutableString alloc] initWithString:[NSString stringWithFormat:@"CREATE TABLE %@(",tableName]];
        int count = 0;
...

{
    ...
    FMResultSet *result =  [db executeQuery:[NSString stringWithFormat:@"SELECT * FROM %@",tableName]];//@"SELECT * FROM %@ LIMIT 10"
