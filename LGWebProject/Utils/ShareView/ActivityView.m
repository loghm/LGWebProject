...
- (void)setup
{
    ...
    self.bgColor = [UIColor colorWithRed:254/255.0 green:254/255.0 blue:254/255.0 alpha:1.0f];
...
- (void)updateConstraints
{
    ...
    if (self.title.length > 0) {
        views = @{@"contentView": self.contentView, @"closeButton": self.closeButton, @"titleLabel": self.titleLabel, @"iconView": self.iconView, @"view": self, @"referView": self.referView};
    } else {
        views = @{@"contentView": self.contentView, @"closeButton": self.closeButton, @"iconView": self.iconView, @"view": self, @"referView": self.referView};
    }
    ...
    if (self.title.length > 0) {
        constraints = [NSLayoutConstraint constraintsWithVisualFormat:@"V:|-8-[titleLabel(==30)]-[iconView]-8-|" options:0 metrics:nil views:views];
        [self.contentView addConstraints:constraints];
    } else {
